...
const REF = {
  ...
  // pumped-architecture departure budget (tmp/ro/i5_refs.py): √(μ⊕/a) escape + v∞ + tax(v∞)
  // (tax = anchored-optimised schedule under the DERIVED THERMAL power model, issue #5;
  ...
  th_cap042_ref: 3.5360714970813185,       // Python EXACT cap_eff(0.42); JS interpolates its
                                           // table -> compare at 1e-6 (table error ~3.5e-7)
  th_t042_ref: 492.4524097153114,
  sched_vinf_ref: 23650.981001347922,
  sched_dv_ref: 24436.558766052716,
  sched_yr_ref: 12.003472637264771,
...
check("pump tax, cap-model optimised schedule, at the 23.64 km/s anchor (m/s)", F.pumpTaxFor(23.64e3, "optimized"), -509.0, 1e-12);
check("pump tax, bang-bang schedule, at the 23.64 km/s anchor (m/s)", F.pumpTaxBangbang(23.64e3), 2000.0, 1e-12);
// Anchored campaign lookup (OPT_CAMPAIGN_THERMAL_TABLE interpolation) matches Python campaign_at.
const oc = F.optCampaignFor(23.71e3);
check("anchored thermal campaign Δv at v∞=23.71 km/s (m/s)", oc.dv, 24470.863636363636, 1e-12);
check("anchored thermal campaign custody at v∞=23.71 km/s (yr)", oc.years, 12.046272727272727, 1e-12);
check("anchored thermal campaign revolutions at v∞=23.71 km/s", oc.revs, 7.888909090909091, 1e-12);
// Thermal derating model: derived power curve + cell temperature (issue #5), and the JS
// scheduled integrator flying the anchored thermal campaign end-to-end.
check("derived thermal cap at the 0.42 AU floor", F.capEff(0.42), REF.th_cap042_ref, 1e-6);
check("equilibrium cell temperature at 0.42 AU (K)", F.cellTemperature(0.42), REF.th_t042_ref, 1e-9);
const sg = F.scheduledPumpedVinf(2.5e-4, 23.64e3);
check("scheduled thermal campaign v∞ at the design a0 (m/s)", sg.vinf, REF.sched_vinf_ref, 1e-6);
check("scheduled thermal campaign Δv at the design a0 (m/s)", sg.dv, REF.sched_dv_ref, 1e-6);
check("scheduled thermal campaign custody at the design a0 (yr)", sg.years, REF.sched_yr_ref, 1e-6);
