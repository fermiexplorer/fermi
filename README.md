...
| Best departure window | **two separate mission variations, each with its own design epoch**: direct ~72,800 yr (fuel optimum, PSI-corroborated). Pumped: a **flat design window ~73k–79.3k yr** (fuel basin bottoms near ~77,500 yr on the deep-simulation grid — sub-noise, see `docs/PP-ARRIVAL-OPTIMUM.md`; the geometry-anchored 79,250-yr ecliptic crossing, +27 m/s — model noise — is the fuel/robustness end and the slider default; the ~73k end costs +0.27 km/s ≈ ~$25k but arrives ~6,000 yr sooner — the arrival-value end). PSI did not derive the pumped fuel optimum (their ~73,000 reuses the impulsive optimum; tilt pricing flagged open in their report — our 3-D pricing settles it); the calculator snaps the arrival slider to the selected architecture's design point |
