...
| **Pumped SEP departure Δv (default architecture)** | ~32.3 km/s two-leg total at its 79,766-yr crossing design point (anchored optimised schedule under a DERIVED thermal power curve, cap_eff(0.42 AU)=3.54, + a DERIVED 3-D out-of-plane steering cost — ~0 at the in-plane crossing aim, 0.51 km/s at the 2.48° direct-optimum aim; at PSI's idealised 4× cap the same construction gives 23.14 vs their 23.97 — 3.5% better); closes at today's α (~15–21 W/kg) |
| Best departure window | **two separate mission variations, each with its own design epoch**: direct ~73,100 yr (fuel optimum, PSI-corroborated). Pumped: a **flat design window ~73k–79.8k yr** (fuel basin bottoms near ~77,500 yr on the deep-simulation grid — sub-noise, see `docs/PP-ARRIVAL-OPTIMUM.md`; the geometry-anchored **79,766-yr ecliptic crossing** — 79,756 yr from the 2029 departure, on the jointly-adopted Akeson-J2019.5 kinematic state — +35 m/s, model noise, is the fuel/robustness end and the slider default; the ~73k end costs +0.25 km/s ≈ 2.1 kg of xenon ≈ ~$23k but arrives ~6,800 yr sooner — the arrival-value end). PSI did not derive the pumped fuel optimum (their ~73,000 reuses the impulsive optimum; tilt pricing flagged open in their report — our 3-D pricing settles it); the calculator snaps the arrival slider to the selected architecture's design point |
| Departure aim | ~2.4° off the ecliptic (direct optimum); 0° (in-plane) at the pumped crossing design point |
| Reference vehicles | pumped default ~155 kg wet @2 kW today's-class hardware at its ~77.5k basin bottom (91 W/kg array, GaAs cells — the 0.42 AU passes require GaAs-class derating); direct spiral needs the far-term high-α preset (~43 kg wet @1000 W/kg at its ~73.1k optimum); conservative direct ~600 kg wet @5 kW silicon |
...
# UI behaviour: every slider drives the right outputs, in the right direction (~93 checks)
